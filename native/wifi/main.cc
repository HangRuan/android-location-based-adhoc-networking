...
int config() {
  char  iflan[IFNAMSIZ];
  in_addr_t lan_gw = inet_addr("192.168.13.1");
...
IfCtl ic("eth0");
  ...
  in_addr_t lan_gw = inet_addr("192.168.13.12");
  in_addr_t lan_netmask = inet_addr("255.255.255.0");
  (ic.setAddress(lan_gw)
               && ic.setMask(lan_netmask)
               && ic.setState(true));
...
printf("Creating IwCtl\n");
  IwCtl ic("eth0");
  if(ic.setState(true)) { // just in case
    ic.setChannel(channel); // ignore return value

printf("Called ic.set\n");
    FILE *f = fopen("wpa.conf", "w");
    if (f) {
    printf("Inside if (f) \n");
      fprintf(f, "ctrl_interface=%s\nap_scan=2\n", iflan);
